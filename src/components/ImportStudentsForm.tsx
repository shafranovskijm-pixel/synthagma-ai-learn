...
export default function ImportStudentsForm({ organizationId, courses, onSuccess }: ImportStudentsFormProps) {
  ...
  const parseCSV = (text: string): { email: string; name: string }[] => {
    ...
    for (let i = startIndex; i < lines.length; i++) {
      ...
      if (parts.length >= 2) {
        // Try to determine which column is email
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (students.length === 0) {
        toast.error("Не найдено записей для импорта. Проверьте формат файла.");
      ...
      if (failCount === 0) {
        toast.success(`Успешно импортировано ${successCount} учеников`);
      } else if (successCount === 0) {
        toast.error(`Ошибка импорта всех ${failCount} учеников`);
      } else {
        toast.warning(`Импортировано ${successCount} из ${students.length} учеников`);
      }
    } catch (error: any) {
    ...
    return (
      <div className="space-y-4 py-4">
        <div className="flex items-center gap-4 p-4 bg-secondary/50 rounded-xl">
          <div className="flex items-center gap-2 text-sigma-green">
